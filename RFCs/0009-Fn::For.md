...
* (Optional) `LogicalId`
    ...
    * The name should be interpolated with an index using `Fn::Sub` intrinsic function.
    * If `Fn::For` is declared to replicate Resource or Output object, this parameter is required, otherwise, you will get an error message saying `Fn::For` requires a `LogicalId` parameter.
